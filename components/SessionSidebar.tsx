...
function PiAgentTitle() {
  ...
  const target = showVersion ? `${process.env.NEXT_PUBLIC_APP_VERSION ?? "0.0.0"}p${process.env.NEXT_PUBLIC_PI_VERSION ?? "0.0.0"}` : "Mindora";
  ...
  useEffect(() => {
    ...
    if (selectedCwd === null) {
      ...
      if (projects.length > 0) {
        setSelectedCwd(projects[0]);
      } else {
        // No sessions yet — auto-pick a default workspace directory
        fetch("/api/default-cwd", { method: "POST" })
          .then((res) => res.json().catch(() => ({})))
          .then((data: { cwd?: string; error?: string }) => {
              ...
              {!customPathOpen ? (
                ...
                <div style={{ padding: "6px 8px", borderTop: visibleProjects.length > 0 ? "none" : undefined }}>
                  <input
                    ...
                    placeholder={t("sidebar.custom_path_placeholder")}
                    style={{
                      width: "100%",
                      fontSize: 11,
                      fontFamily: "var(--font-mono)",
              ...
              >
                        ...
                        >
                          ...
                          {!wt.isMain && (
                            <button
                              ...
                              disabled={wtBusy}
                              title={t("sidebar.remove_worktree", undefined, { path: wt.path })}
                              style={{
                                display: "flex", alignItems: "center", justifyContent: "center",
                  ...
                  {!wtNewOpen ? (
                    <button
                      ...
                      title={t("sidebar.create_worktree")}
                      style={{
                        display: "flex",
                        alignItems: "center",
                        gap: 7,
...
function RunningSessionIndicator() {
  ...
  return (
    <span
      title={t("sidebar.agent_running")}
      aria-label={t("sidebar.agent_running")}
      style={{
        width: 14,
        height: 14,
        display: "inline-flex",
...
function UnreadSessionIndicator() {
  ...
  return (
    <span
      title={t("sidebar.new_activity")}
      aria-label={t("sidebar.new_session_activity")}
      style={{
        width: 14,
        height: 14,
        display: "inline-flex",
        ...
        <>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div
              ...
              title={isRunning ? t("sidebar.session_running_suffix", undefined, { title }) : isUnread ? t("sidebar.session_unread_suffix", undefined, { title }) : title}
            >
            ...
            <div style={{ marginTop: 2, display: "flex", gap: 8, color: "var(--text-dim)", fontSize: 11, minWidth: 0 }}>
              ...
              {session.worktreeBranch && (
                <span
                  title={t("sidebar.worktree_label", undefined, { path: session.cwd })}
          ...
          {hasChildren && (
            <button
              ...
              title={collapsed ? t("sidebar.expand_forks") : t("sidebar.collapse_forks")}
              style={{
                display: "flex", alignItems: "center", justifyContent: "center",
